<template>    
   <div>
    <Header />
    <div>
       <h2 style="font-size:60px">Mascotas</h2>
  ...
  <h2 align = left>Nueva Mascota</h2>
  <form v-on:submit.prevent="nuevaMascota">
    <label for="exampleInputEmail1" class="input">Dueño</label>
    <div class="row">
      <div class="col-6">
        <div class="select1">
          <select name="cliente" aria-label="Default select example" v-model="mascotaNueva.cliente">
            <option v-for="clientes in selectClientes" :key="clientes.id" :value="clientes.id" >{{clientes.nombre}}</option>
          </select>
        </div>
      </div>
      <div class="col-6">
        <input type="text" placeholder="Nombre...." class="input1" id="nombre" aria-describedby="nombre" v-model='mascotaNueva.nombre'>
      </div>
    </div>
    <br>
    <div class="row">
      <div class="col-6">
        <input type="text" class="input1" id="direccion" aria-describedby="direccion" placeholder="Edad...." v-model='mascotaNueva.edad'>
      </div>
      <div class="col-3">
        <input type="text" class="input1" id="telefono" aria-describedby="telefono" placeholder="Raza...." v-model='mascotaNueva.raza'>
      </div>
    </div>
    <br>
    <div class="mb-3">
      <input type="text" class="input1" id="telefono" aria-describedby="telefono" placeholder="Tipo...." v-model='mascotaNueva.tipo'>
    </div>
...
<div class="container izquierda">
                ...
                <thead>
                    <tr>
                      ...
                      <th scope="col" style="text-align:center">HISTORIAL CLINICO</th>
                    </tr>
                </thead>
                <tbody>
                    <tr v-for="mascotas in todos" :key="mascotas.id" v-animate-css.hover="'headShake'" v-animate-css.click="'fadeOutLeft'">
                        <th scope="row">{{mascotas.m_id}}</th>
                        <th>{{mascotas.m_nombre}}</th>
                        <th>{{mascotas.m_raza}}</th>
                        <th>{{mascotas.m_tipo}}</th>
                        <th>{{mascotas.m_edad}}</th>
                        <th>{{mascotas.c_nombre}}</th>
                        <th style="text-align:center; width:200px;"><button id="btncancelar" class="btn btn-warning" v-on:click="uno(mascotas.m_id)">Actualizar</button></th>
                        <th style="text-align:center; width:190px;"><button id="btncancelar" class="btn btn-danger"  v-on:click="deleteOne(mascotas.m_id)">Eliminar</button></th>
                        <th style="text-align:center; width:190px;"><button id="btncancelar" class="btn btn-danger"  v-on:click="downloadPDF(mascotas.m_id)">Generar</button></th>
...
<div class="container" v-if="cambio" v-animate-css="'zoomIn'">
  ...
  <form v-on:submit.prevent="update">
    <div class="col-3" hidden>
        ...
        <input type="text" class="input1" id="id" aria-describedby="id" v-model='mascota.id'>
    </div>
    <div class="row">
      <div class="col-6">
        <label for="exampleInputEmail1" class="form-label">Nombre</label>
        <input type="text" class="input1" id="nombre" aria-describedby="nombre" v-model='mascota.nombre'>
      </div>
      <div class="col-6">
        <label for="exampleInputEmail1" class="form-label">Raza</label>
        <input type="text" class="input1" id="direccion" aria-describedby="direccion" v-model='mascota.raza'>
      </div>
    </div>
    <div class="row">
      <div class="col-6">
        <label for="exampleInputEmail1" class="form-label">Tipo</label>
        <input type="text" class="input1" id="telefono" aria-describedby="telefono" v-model='mascota.tipo'>
      </div>
      <div class="col-6">
        <label for="exampleInputEmail1" class="form-label">Edad</label>
        <input type="text" class="input1" id="telefono" aria-describedby="telefono" v-model='mascota.edad'>
      </div>
    </div>
    <br>
    <div class="row">
...
export default {
  ...
  data() {
    return{
      ...
      cliente_id:"",
      error_msg:"",
      mascotaNueva:{nombre:"",raza:"",tipo:"",edad:"",cliente:""},
  ...
  methods:{
    botonNuevo(){
      if (this.nuevo){
          ...
          this.mascotaNueva={nombre:"",raza:"",tipo:"",edad:"",cliente:""}
      }else{
          this.nuevo = true
...
if(this.mascotaNueva.cliente=="" || this.mascotaNueva.nombre ==""|| this.mascotaNueva.raza ==""|| this.mascotaNueva.tipo ==""|| this.mascotaNueva.edad ==""){
        console.log("cancelo")
        this.error=true;
      ...
      this.error=false;
      axios
      .post(this.URL+'mascotas',this.mascotaNueva,{
        headers: {
              'Authorization': `Bearer ${localStorage.token}`
                }
        })
      ...
      axios
        ...
        headers: {
          ...
          .then(response => {
            ...
            this.mascota=response.data.Cliente
            this.cambio = true
          })
          .catch( e=> console.log(e))
    },
    uno($id){
      axios
        .get(this.URL+'mascotas/'+$id,{
        headers: {
              'Authorization': `Bearer ${localStorage.token}`
                }})
          .then(response => {
            
            this.mascota=response.data.Mascota[0]
    ...
    deleteOne($num){
      axios
        .put(this.URL+'mascotas/changeStatus/'+$num,{
        headers: {
              'Authorization': `Bearer ${localStorage.token}`
                }
        })
    ...
    update(){
      axios
      .put(this.URL+'mascotas/'+this.mascota.id,this.mascota,{
        headers: {
              'Authorization': `Bearer ${localStorage.token}`
                }
        })
